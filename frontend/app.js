...
async function searchSongs() {
    ...
    try {
        ...
        addSongs(data.songs);
        status.textContent = `${data.songs.length} músicas adicionadas à fila.`;
    } catch (error) {
...
async function importTxtFile() {
    ...
    try {
        const response = await fetch(`${API}/api/import-file`, {
            method: "POST",
            body: formData
        });
        const data = await response.json();
        ...
        status.textContent = "Erro ao importar o arquivo TXT.";
        console.error(error);
    } finally {
...
function handleImportResult(data) {
    ...
    let message = `${added} músicas adicionadas à fila.`;
    if (data.errors?.length) {
        message += ` ${data.errors.length} link(s) não puderam ser lidos.`;
    }
        ...
        const poll = async () => {
            try {
                ...
                if (data.status === "completed") {
                    ...
                    currentSong.textContent = "Concluído.";
